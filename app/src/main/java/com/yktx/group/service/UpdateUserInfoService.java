...
public class UpdateUserInfoService extends Service{
	...
	void httpFail(String erro) {
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.UPDATEUSERINFO);
...
//	 		reponse = {"statusCode":"1","mess":"验证验证码:验证成功","data":{"pushallas":"b1e00bfbb53d4ea58e268683035b589d"}}
			...
			if (Contanst.HTTP_SUCCESS.equals(retcode)) {//成功获取数据
				...
				serviceListener.getJOSNdataSuccess(userBean, retcode , Contanst.UPDATEUSERINFO);
			} else {
				String errmsg = (String) result.get("mess");
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.UPDATEUSERINFO);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.UPDATEUSERINFO);
			e.printStackTrace();
		}
	}
