 ...
 *
 */
public class RegisterVerificationService extends Service{

	
	public RegisterVerificationService(String requestType,
			Hashtable<String, String> params, String urlParams,
			ServiceListener serviceListener) {
		super(requestType, params, urlParams, serviceListener);
		this.url=UrlParams.IP+Contanst.HTTP_REGISTER_VERIFCATION;
		Log.i(  "bbb", "url ===== "+url);
	}

	@Override
	void httpFail(String erro) {
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.REGISTER_VERIFCATION);
...
//		LodingActivity.isJion = false;
		try {
			...
			if (Contanst.HTTP_SUCCESS.equals(retcode)) {//成功获取数据
				...
				serviceListener.getJOSNdataSuccess(userBean, retcode , Contanst.REGISTER_VERIFCATION);
			} else {
				String errmsg = (String) result.get("mess");
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.REGISTER_VERIFCATION);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.REGISTER_VERIFCATION);
			e.printStackTrace();
		}
	}
