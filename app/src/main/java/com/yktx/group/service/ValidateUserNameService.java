 ...
 *
 */
public class ValidateUserNameService extends Service{

	public ValidateUserNameService(String requestType,
			Hashtable<String, String> params, String urlParams,
			ServiceListener serviceListener) {
		super(requestType, params, urlParams, serviceListener);
		this.url=UrlParams.IP+Contanst.HTTP_VALIDATEUSERNAME
				+".html";
		Log.i(  "aaa", "url ===== "+url);
	}
	...
	void httpFail(String erro) {
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.VALIDATEUSERNAME);
...
//			HomeBestView.listCount = Integer.parseInt((String)result.get("tot_count")); 
			...
			if (Contanst.HTTP_SUCCESS.equals(retcode)) {//成功获取数据
				serviceListener.getJOSNdataSuccess("ok", retcode , Contanst.VALIDATEUSERNAME);
			} else {
				String errmsg = (String) result.get("mess");
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.VALIDATEUSERNAME);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.VALIDATEUSERNAME);
			e.printStackTrace();
		}
	}
