...
/**
 * 来自：http://www.sunjs.com 老孙个人博客 更多精彩请继续关注老孙个人博客
 * 
 * @author Kevon.sun 2013-10-25 14:15:16
 */
public class Geohash {
	...
	public static Map BORDERS = new HashMap();
	public static Map NEIGHBORS = new HashMap();
	...
	public static String calculateAdjacent(String srcHash, String dir) {
		...
		if (((String) BORDERS.get(dir + ":" + type)).indexOf(lastChr) != -1) {
			base = calculateAdjacent(base, dir);
		}
		base = base
				+ BASE32.toCharArray()[(((String) NEIGHBORS.get(dir + ":"
						+ type)).indexOf(lastChr))];
		return base;
	}
	...
	public static void expandLngLat(String geohash, int len) {
		...
		String centerPoint = (lat[0] + lat[1]) / 2 + "," + (lon[0] + lon[1])
				/ 2;
	 ...
	 * 获取当前的区域和周围8个区域的位置编码。
	 * */
	public static List<String> coorMapBylnglat(double lat, double lon) {
	...
	/**
	 * 获取当前的区域和周围8个区域的位置编码。并规定编码的长度用于精准、
	 * */
	public static List<String> coorMapBylnglat(double lat, double lon,
			int geohashLen) {
	...
	/**
	 * 根据经纬度获取geohash码，精确位置
	 * */
	public static String getGeoHash(double lat, double lon, int geohashLen) {
		String geohash = encode(lat, lon).substring(0, geohashLen);
		return geohash;
	}
	...
	public static void main(String[] args) {
		...
		double lata = 39.9383;
		double lona = 116.42767; // 需要查询经纬度，目前指向的是BeiJing
		double latb = 39.945545;
		double lonb = 116.43935; // 需要查询经纬度，目前指向的是BeiJing

		System.out.println(GetDistance(lata, lona, latb, lonb));
	}
