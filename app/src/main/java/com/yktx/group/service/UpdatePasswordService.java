...
public class UpdatePasswordService extends Service{
	...
	void httpFail(String erro) {
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.UPDATEPASSWORD);
...
//			HomeBestView.listCount = Integer.parseInt((String)result.get("tot_count")); 
			...
			if (Contanst.HTTP_SUCCESS.equals(retcode)) {//成功获取数据
				serviceListener.getJOSNdataSuccess("ok", retcode , Contanst.UPDATEPASSWORD);
			} else {
				String errmsg = (String) result.get("mess");
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.UPDATEPASSWORD);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.UPDATEPASSWORD);
			e.printStackTrace();
		}
	}
